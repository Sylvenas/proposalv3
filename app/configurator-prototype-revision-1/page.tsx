...
export default function ConfiguratorPrototypePage() {
  ...
  // Append a library product as a custom line item; returns the new row's fee
  // id (custom items share the fee maps). Used by both "Add Other Items" and
  // "Add Addons" (the latter also flags the new row as an add-on).
  const addCustomItem = (libId: string, qty: number): string | null => {
    ...
    setCustomItems((prev) => [
      ...prev,
      {
        ...
        unitPrice: perUnit,
        unitLabel: label,
        // Real products keep their legend swatch (explicit color, or the
        // flooring hatch derived from the name); only visual-less services /
        // fees fall back to the crossed-out placeholder like the preset rows.
        thumb: it.color ?? (it.refArea != null ? thumbBackground(it.name) : 'none'),
      },
    ]);
    return id;
  };
            ...
            {reportOpen && (
              <ReportSheet
                ...
                onAddOtherItem={(libId, qty) => {
                  addCustomItem(libId, qty);
                }}
                onAddAddonItem={(libId, qty) => {
                  const id = addCustomItem(libId, qty);
                  if (id) setFeeAddon((prev) => ({ ...prev, [id]: true }));
                }}
...
function ReportSheet({
  ...
  onAddAddonItem: (libId: string, qty: number) => void;
  onClose: () => void;
}) {
  ...
  // "Add Other Items" / "Add Addons" flow: pick a product (page 1) → enter a
  // quantity (page 2) → append it to the summary as a line item. The source
  // button decides where the new row lands: "Add Addons" flags it as an
  // add-on immediately (no add-then-"Set as Add-on" round trip).
  const [addStep, setAddStep] = useState<'closed' | 'pick' | 'qty'>('closed');
  const [addSource, setAddSource] = useState<'other' | 'addon'>('other');
          ...
          <div style={{ flex: '1 1 auto', minWidth: 0, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            >
              <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 28 }}>
                <button
                  type="button"
                  onClick={() => { setAddSource('other'); setAddStep('pick'); }}
                ...
                <button
                  type="button"
                  onClick={() => { setAddSource('addon'); setAddStep('pick'); }}
                  style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'transparent', border: 'none', color: BLUE, fontSize: 17, cursor: 'pointer', padding: 0 }}
                >
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke={BLUE} strokeWidth="1.8" strokeLinecap="round">
                    <path d="M9 3v12M3 9h12" />
                  ...
                  <>
                    ...
                    {addStep === 'qty' && pickedItem && (
                      <OtherItemQuantityCard
                        ...
                        onCancel={closeAdd}
                        onAdd={() => {
                          (addSource === 'addon' ? onAddAddonItem : onAddOtherItem)(pickedItem.id, qty);
                          closeAdd();
                        }}
                        placement={{ top: 'calc(100% + 12px)', left: 0 }}
                      />
...
/** Radio indicator for upgrade option rows — exact Figma asset geometry
 *  (node 1676:29439 selected / 1676:29442 unselected). */
function HoRadio({ selected, size = 18 }: { selected: boolean; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden style={{ flexShrink: 0, display: 'block' }}>
      <circle cx="9" cy="9" r="8.25" stroke="#262626" strokeWidth="1.5" />
      {selected && <circle cx="9" cy="9" r="5" fill="#262626" />}
    </svg>
  );
}

/** A single upgradeable / plain product line item. The top row opens the
 *  product (or upgrade) detail sheet, matching the proposal-v3 behavior.
 *  Upgradeable items list every option below the row (Figma 1676:30888):
 *  radio + option title + the price delta relative to the current choice
 *  ("Selected" on the current one). */
function HoProductLine({
  ...
  measure: string;
  onOpen?: () => void;
  options?: HoUpgradeOption[];
  selectedOptionId?: string;
  onSelectOption?: (id: string) => void;
}) {
  const selDelta = options?.find((o) => o.id === selectedOptionId)?.delta ?? 0;
  return (
    <div style={{ borderTop: '0.5px solid rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', padding: '12px 0', width: '100%' }}>
      <div
        onClick={onOpen}
        style={{
          display: 'flex',
      ...
      {options && options.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginLeft: 56, marginTop: 12, padding: '8px 0 8px 16px', background: 'rgba(0,0,0,0.02)', borderRadius: 8 }}>
          {options.map((o) => {
            const selected = o.id === selectedOptionId;
            const rel = o.delta - selDelta;
            return (
              <div
                key={o.id}
                onClick={() => onSelectOption?.(o.id)}
                style={{ display: 'flex', gap: 8, alignItems: 'center', height: 40, width: '100%', cursor: onSelectOption ? 'pointer' : 'default' }}
              >
                <div style={{ width: 18, flex: '0 0 auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <HoRadio selected={selected} />
                </div>
                <p style={{ flex: '1 1 0', minWidth: 0, fontSize: 16, color: PROP_INK, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {o.title}
                </p>
                {/* Same 130px column as the top row's quantity so the price
                    left-aligns under the measure text; trailing 48px spacer
                    mirrors the info-icon column. */}
                <div style={{ width: 130, flex: '0 0 auto', display: 'flex', gap: 2, alignItems: 'center', fontSize: 16, color: PROP_INK, whiteSpace: 'nowrap' }}>
                  {selected ? (
                    <span>Selected</span>
                  ) : (
                    <>
                      <span style={{ fontWeight: 300 }}>{rel < 0 ? '-$' : '+$'}</span>
                      <span style={{ fontWeight: 300 }}>
                        {Math.abs(rel).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </>
                  )}
                </div>
...
function HoButton({ children, variant = 'outline' }: { children: React.ReactNode; variant?: 'outline' | 'primary' }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', fontFamily: PROP_FONT, color: PROP_INK }}>
      ...
      <div style={{ flex: '1 1 auto', minHeight: 0, overflowY: 'auto', background: '#fff' }}>
        {/* page header */}
        <header style={{ height: 56, background: '#fff', borderBottom: '0.5px solid rgba(0,0,0,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 32px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', maxWidth: 1160 }}>
            <HomeIcon size={24} />
            <FloorLogo />
            <UserIcon size={24} />
          </div>
        ...
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '24px 24px 48px', display: 'flex', gap: 24, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: '2 1 0', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
            {/* Drawing — the exact CAD plan built on the iPad */}
            <HoSectionCard label="Drawing">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%' }}>
                <div
                  onMouseEnter={() => setDrawingHover(true)}
                  onMouseLeave={() => setDrawingHover(false)}
                  style={{ position: 'relative', width: '100%', aspectRatio: '1120 / 590', background: '#fff', border: '1px solid #ececec', borderRadius: 4, overflow: 'hidden' }}
                >
                  <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
                    <FloorPlanSvg selected={null} onSelect={() => {}} viewBox="0 135 1120 590" productOverrides={drawingOverrides} />
                  </div>
            ...
            {/* Included Products — from the app product summary */}
                    ...
                    {c.rows.map((row) => {
                      ...
                      return (
                        <HoProductLine
                          ...
                          onOpen={() => setDetail({ section: 'included', cat: c.category, key: k })}
                          options={hasUpgrade ? opts : undefined}
                          selectedOptionId={hasUpgrade ? selId : undefined}
                          onSelectOption={hasUpgrade ? (id) => setHoUpgrade((prev) => ({ ...prev, [k]: id })) : undefined}
                        />
                      );
                    })}
                  </div>
            ...
            {/* Optional Add-ons — from products flagged "Set as Add-on" */}
            {addonCats.length > 0 && (
          ...
          <div style={{ flex: '1 1 0', minWidth: 0, position: 'sticky', top: 24, alignSelf: 'flex-start' }}>
            <div style={{ padding: '0 12px', display: 'flex', flexDirection: 'column', gap: 24 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                ...
                <p style={{ fontSize: 24, fontWeight: 600 }}>{PROPOSAL_TITLE}</p>
